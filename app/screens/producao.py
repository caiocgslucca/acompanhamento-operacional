...
def page():
    filters=''.join(_filter(key,label) for key,label in (('empresa','Filial'),('classe','Classe'),('data','Data oficial'),('turno','Turno'),('rota','Rota')))
...
def data(empresa:list[str]=Query([]),classe:list[str]=Query([]),data:list[str]=Query([]),turno:list[str]=Query([]),rota:list[str]=Query([])):
    try:
        result=build({'empresa':empresa,'classe':classe,'data':data,'turno':turno,'rota':rota});source=next((s for s in store.list_sources() if clean_name(s['name'])=='CARTEIRA'),None)
...
def pdf(empresa:list[str]=Query([]),classe:list[str]=Query([]),data:list[str]=Query([]),turno:list[str]=Query([]),rota:list[str]=Query([])):
    from reportlab.lib.pagesizes import A2,landscape
    from reportlab.lib import colors
    from reportlab.pdfgen import canvas
    from reportlab.platypus import Table,TableStyle
    selected={'empresa':empresa,'classe':classe,'data':data,'turno':turno,'rota':rota};result=build(selected);buffer=BytesIO();page=landscape(A2);c=canvas.Canvas(buffer,pagesize=page,pageCompression=1);width,height=page
    ...
    filter_text=[]
    for label,key in (('Filial','empresa'),('Classe','classe'),('Data oficial','data'),('Turno','turno'),('Rota','rota')):filter_text.append(f"{label}: {', '.join(selected[key]) if selected[key] else 'Todos'}")
