...
def configured_sources(config):
    explicit=config.get('sources') or {}
    return explicit if explicit else database_sources()
...
def send(client,server,key,name,path_text,previous,log):
    ...
    current=fingerprint(files,base)
    if previous.get(remote)==current:log.info('SEM_ALTERACAO | fonte=%s | arquivos=%s',name,len(files));return previous
    ...
    try:
        ...
        previous[remote]=current;log.info('ENVIO_CONCLUIDO | fonte=%s | arquivos=%s | %s',name,body.get('files'),body.get('message','publicado'))
        return previous
...
def run_once(config,log):
    ...
    if not sources:raise ValueError('Nenhuma fonte local válida foi encontrada. Cadastre as fontes no sistema local ou informe-as no sync_config.json.')
    ...
    with httpx.Client(timeout=httpx.Timeout(600,connect=30),follow_redirects=True) as client:
        ...
        for name,path in sources.items():
            try:state=send(client,server,key,name,path,state,log);save_state(state)
            except Exception as exc:log.exception('ENVIO_FALHOU | fonte=%s | erro=%s',name,exc)
...
def main():
    ...
    while True:
        ...
        except Exception as exc:log.exception('CICLO_FALHOU | erro=%s',exc)
        if once:return
